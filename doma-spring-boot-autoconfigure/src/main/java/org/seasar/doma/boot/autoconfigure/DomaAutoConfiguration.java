...
public class DomaAutoConfiguration {
	...
	public EntityListenerProvider entityListenerProvider(
			DomaEventEntityListener domaEventEntityListener,
			TryLookupEntityListenerProvider tryLookupEntityListenerProvider) {
		return new EntityListenerProvider() {
			@Override
			@SuppressWarnings({ "unchecked" })
			public EntityListener get(Class listenerClass, Supplier listenerSupplier) {
				if (listenerClass == NullEntityListener.class) {
					return tryLookupEntityListenerProvider.get(listenerClass,
							listenerSupplier);
				}
				return domaEventEntityListener;
			}
		};
	}
